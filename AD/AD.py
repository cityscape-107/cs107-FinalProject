...
class AD:
    ...
    def __init__(self, value, der=0, name=("x", 0)):  # changed from der=[0]
        ...
        if isinstance(other, AD):
            ...
            for name_2 in names_2:
                ...
                if name_2 in names_1:
                    index_1 = np.argwhere(names_1 == name_2)
                    self.der[:, index_1] = self.der[:, index_1] + other.der[:, index_2]
                else:
                    self.der = np.concatenate((self.der, other.der[:, index_2]), axis=1)
                    self.name.append(name_2)
            value = self.val
            derivative = self.der
            name = self.name
        else:
